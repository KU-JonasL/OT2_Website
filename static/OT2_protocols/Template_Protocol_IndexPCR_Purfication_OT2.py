...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for k in range(2): # Double wash
        ...
        for i in range(Col_Number):
            ...
            m200.dispense(volume = 190, location = Sample_Plate.wells()[Column].top(z = 1.2), rate = 1) # Dispenses ethanol from 1.2 mm above the top of the well.
    ...
    for i in range(Col_Number):
        Column = i*8 #Gives the index for the first well in the column
        m200.pick_up_tip()
        m200.transfer(volume = 35, source = Ebt, dest = Sample_Plate.wells()[Column], rate = 1, trash = False , new_tip = 'never', mix_after = (5,20))
        #m200.aspirate(volume = 40, location = Ebt, rate = 1)
        #m200.dispense(volume = 40, location = Sample_Plate.wells()[Column], rate = 1)
        #m200.mix(repetitions = 5, volume = 30, location = Sample_Plate.wells()[Column])
        m200.dispense(volume = 20, location = Sample_Plate.wells()[Column], rate = 0.7)
        protocol.delay(5)
